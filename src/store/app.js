 ...
 * La configuración de API_URL y la función apiFetch se han centralizado en `core.js`.
 * Este archivo ahora asume que `core.js` se carga primero en el HTML,
 * proporcionando una única fuente de verdad para las llamadas a la API.
 */
...
// Referencias a los contenedores que ahora son dinámicos
...
async function fetchProducts() {
  try {
    ...
    // if (window.FeaturedModule) window.FeaturedModule.render(); // Comentado, ya que la lógica se movió aquí.
  ...
  const featured = [
    {
      id: "P003",
      name: "BOMBER URBANO",
      description: "Diseño exclusivo con detalles reflectivos. Stock limitado para los que van primero.",
    ...
    {
      id: "P007",
      name: "SET DEPORTIVO W",
      description: "Top + legging. El conjunto ganador para entrenar con el estilo que te define.",
      badge: "✦ Más Vendido",
  ...
  featuredGrid.innerHTML = featured.map(item => `
    <div class="featured-item reveal">
      ...
      <div class="featured-content">
        <div class="featured-tag">${item.badge}</div>
        <div class="featured-name">${item.name}</div>
        <div class="featured-desc">${item.description}</div>
        <div class="size-selector" data-product="${item.id}">
          ${item.sizes.map((size, index) => `
            <button class="size-btn ${index === 1 ? 'active' : ''}" data-size="${size}">${size}</button>
          `).join('')}
        </div>
        <button class="featured-cta" onclick="addToCart('${item.id}')">
          Agregar al carrito →
        </button>
      </div>
    </div>
...
function renderCategories() {
  ...
  const categories = [
    { name: "MUJER", filter: "mujer", img: "https://images.unsplash.com/photo-1515886657613-9f3515b0c78f?w=700&q=80" },
    { name: "HOMBRE", filter: "hombre", img: "https://images.unsplash.com/photo-1552374196-1ab2a1c593e8?w=500&q=80" },
    { name: "OUTERWEAR", filter: "hombre", img: "https://images.unsplash.com/photo-1591047139829-d91aecb6caea?w=500&q=80" },
    { name: "ACCESORIOS", filter: "accesorios", img: "https://images.unsplash.com/photo-1622560480605-d83c853bc5c3?w=500&q=80" },
    { name: "CALZADO", filter: "all", img: "https://images.unsplash.com/photo-1542291026-7eec264c27ff?w=500&q=80" },
  ];

  categoriesGrid.innerHTML = categories.map(cat => `
    <div class="cat-card reveal" onclick="filterByCategory('${cat.filter}')">
...
function renderTestimonials() {
  ...
  const testimonials = [
    {
      text: "La calidad superó todas mis expectativas. Las telas son increíbles y el envío llegó en 2 días. Definitivamente mi tienda favorita.",
      author: "Valentina M.",
      location: "Bogotá, Colombia",
      avatar: "https://i.pravatar.cc/80?img=47"
    },
    {
      text: "Compré el bomber y todos me preguntan dónde lo conseguí. Winner tiene los diseños más únicos del mercado colombiano.",
      author: "Sebastián R.",
      location: "Medellín, Colombia",
      avatar: "https://i.pravatar.cc/80?img=12"
    },
    {
      text: "Atención al cliente de 10. Tuve un problema con la talla y lo resolvieron al instante. Calidad premium a precios justos.",
      author: "Daniela C.",
  ...
  testimonialsGrid.innerHTML = testimonials.map(t => `
    <div class="test-card reveal">
      ...
      <div class="test-author">
        <img src="${t.avatar}" alt="${t.author}" class="test-avatar" />
        <div>
