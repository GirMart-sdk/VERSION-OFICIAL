...
async function startScanner() {
  ...
  await html5QrScanner.start(
    ...
    (text) => {
      ...
      } catch (e) { /* Ignorar errores de parseo, usar el texto original */ }
      const p = window.AppStore.state.inventory.find((x) => x.id === data || x.sku === data);
      if (p) {
        if (_scanMode === "pos") {
...
window.processManualQR = () => {
  ...
  const p = window.AppStore.state.inventory.find((x) => x.sku === val || x.id === val);
  if (p) {
