...
function renderPOSProducts() {
  try {
    ...
    const catFilter =
      window.AppStore.state.pos.activeCategory === "all"
        ? ""
        : window.AppStore.state.pos.activeCategory;
...
function setPOSCategory(cat) {
  window.AppStore.state.pos.activeCategory = cat; // Actualizar el estado central
  document
...
function clearPOS() {
  if (window.AppStore.state.pos.cart.length > 0 && !confirm("¿Vaciar venta actual?"))
    return;
  window.AppStore.commit('CLEAR_POS_CART');
...
function openPOSPaymentModal(isLayawayInitially = false) {
  if (!window.AppStore.state.pos.cart || !window.AppStore.state.pos.cart.length) return toast("⚠️ Carrito vacío");
...
async function confirmPOSPaymentWithDetails() {
  ...
  try {
    const sale = {
      id: window.genId(),
      timestamp: window.nowStr(), // Corregido para usar el store
      items: window.AppStore.state.pos.cart,
      total: total,
    ...
    if (res.ok) {
      ...
      printReceipt(sale);
      window.AppStore.commit('CLEAR_POS_CART'); // Limpiar carrito a través de una mutación
      renderPOSCart();
