...
// Suscribirse a los cambios del store para actualizar el dashboard automáticamente
window.AppStore.subscribe(state => {
  if (window.WinnerApp.state.activePage === 'dashboard') {
    // Solo actualiza si el dashboard es la página activa
    renderLiveActivity(state.salesLog);
  }
});
...
async function renderDashboard() {
  ...
  // Forzar la primera carga de datos de ventas
...
function renderLiveActivity(sales = null) {
  const container = $("dashRecentSales") || $("dashLiveActivity");
  if (!container) return;

  const salesToRender = sales || window.AppStore.state.salesLog || [];
  ...
  container.innerHTML = salesToRender
    .slice(0, 8)
    .map((s, i) => {
